...
"""
Standalone inference script for test_evaluation dataset.

Data path: /mnt/zihanw/proj_utils_pro/transfer_video_maker/output_test_evaluation
Contains 8 test scenes: 031, 033, 053, 056, 076, 077, 088, 089 (seg01 only)
...
Usage:
    ...
    torchrun --nproc_per_node=8 --master_port=12345 \
      -m cosmos_transfer2.experiments.custom.inference_test_evaluation \
...
# ============================================================================
# Dataset paths for test_evaluation
# ============================================================================
TEST_EVAL_BASE_DIR = "/mnt/zihanw/proj_utils_pro/transfer_video_maker/output_test_evaluation"
TEST_EVAL_BLUR_DIR = f"{TEST_EVAL_BASE_DIR}/BlurProjection"
TEST_EVAL_DEPTH_DIR = f"{TEST_EVAL_BASE_DIR}/DepthSparse"
TEST_EVAL_HDMAP_DIR = f"{TEST_EVAL_BASE_DIR}/HDMapBbox"

# Default output directory
DEFAULT_OUTPUT_DIR = "/mnt/zihanw/Output_R2V_world_foundation_model_v1/inference_test_evaluation"

# Camera views (same as training)
TRAINING_CAMERAS = (
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test evaluation dataset inference")
...
def create_test_evaluation_dataset(num_views: int = 7):
    """Create dataset for test_evaluation inference.

    This dataset contains only test scenes (031, 033, 053, 056, 076, 077, 088, 089).
    No scene exclusion needed.
    ...
    logger.info(f"Creating test_evaluation dataset with {num_views} view(s): {camera_keys}")
    logger.info(f"Data paths:")
    logger.info(f"  Blur: {TEST_EVAL_BLUR_DIR}")
    logger.info(f"  Depth: {TEST_EVAL_DEPTH_DIR}")
    logger.info(f"  HDMap: {TEST_EVAL_HDMAP_DIR}")

    dataset = MultiControlMultiviewDataset(
        blur_dataset_dir=TEST_EVAL_BLUR_DIR,
        depth_dataset_dir=TEST_EVAL_DEPTH_DIR,
        hdmap_dataset_dir=TEST_EVAL_HDMAP_DIR,
        ...
        exclude_scene_ids=[],  # No exclusion - use all samples in test_evaluation
    )
    logger.info(f"Test evaluation dataset has {len(dataset)} samples")

    if len(dataset) == 0:
        raise ValueError("No samples found in test_evaluation dataset!")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Test Evaluation Dataset Inference")
    logger.info("=" * 60)
    logger.info(f"Checkpoint: {args.ckpt_path}")
    logger.info(f"Output dir: {args.output_dir}")
    logger.info(f"Data dir: {TEST_EVAL_BASE_DIR}")
    ...
    try:
        ...
        # Create test_evaluation dataset
        test_dataset = create_test_evaluation_dataset(num_views=num_views)
