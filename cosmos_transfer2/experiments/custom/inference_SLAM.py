...
Usage:
    ...
    torchrun --nproc_per_node=8 --master_port=12345 \
        -m cosmos_transfer2.experiments.custom.inference_SLAM \
        --ckpt_path /path/to/checkpoints/iter_000008000 \
...
SLAM_HDMAP_DATASET_DIR = "/mnt/zihanw/merge_dyn_stat/proj_utils_pro_Roadside_transfer/transfer_video_maker/output/HDMapBbox"
...
def save_per_view_results(
    ...
    for v in range(n_views):
        ...
        # Save generated video for this view
...
def main():
    ...
    logger.info(f"SLAM Blur: {SLAM_BLUR_DATASET_DIR}")
    logger.info(f"SLAM Depth: {SLAM_DEPTH_DATASET_DIR}")
    logger.info(f"SLAM HDMap: {SLAM_HDMAP_DATASET_DIR}")
    logger.info("=" * 60)
